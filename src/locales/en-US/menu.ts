export default {
  'menu.AdminUsers': 'Admin Users',
  'menu.AdminUsers.UserAccount': 'User Account',
  'menu.AdminUsers.UserAccount.Center': 'Account Center',
  'menu.AdminUsers.UserAccount.Settings': 'Account Settings',
  'menu.AdminUsers.UserAccount.Messages': 'Messages',
  'menu.AdminUsers.Permissions': 'Permission Management',
  'menu.AdminUsers.Permissions.Roles': 'Role Management',
  'menu.AdminUsers.Permissions.Users': 'User Management',
  'menu.AdminUsers.Permissions.Config': 'Permission Config',
  'menu.AdminUsers.Audit': 'Audit Logs',
  'menu.AdminUsers.LoginLogs': 'Login Logs',
  'menu.AdminUsers.OperationLogs': 'Operation Logs',

  // Support
  'menu.Support': 'Support',
  'menu.Support.Tickets': 'Tickets',
  'menu.Support.TicketDetail': 'Ticket Detail',
  'menu.Support.FAQ': 'FAQ',
  'menu.Support.Feedback': 'Player Feedback',
  'menu.Support.Bugs': 'Bugs',


  // Security settings
  'menu.Security': 'Security',
  'menu.Security.Roles': 'Role Management',
  'menu.Security.Users': 'User Management',
  'menu.Security.Config': 'Permission Config',

  // New game management structure
  'menu.GameManagement': 'Game Management',
  'menu.GameManagement.GameEnvironments': 'Game Environments',
  'menu.GameManagement.GameEntities': 'Entity Management',
  'menu.GameManagement.GameEntities.EntitiesList': 'Entities List',
  'menu.GameManagement.GameEntities.EntityFunctions': 'Entity Functions',
  'menu.GameManagement.FunctionManagement': 'Function Management',
  'menu.GameManagement.FunctionManagement.FunctionCatalog': 'Function Catalog',
  'menu.GameManagement.FunctionManagement.Registered': 'Registered',
  'menu.GameManagement.FunctionManagement.Registered.examples': 'Examples',
  'menu.GameManagement.FunctionManagement.Registered.examples.analytics': 'Analytics',
  'menu.GameManagement.FunctionManagement.Registered.examples.chat': 'Chat',
  'menu.GameManagement.FunctionManagement.Registered.examples.config': 'Config',
  'menu.GameManagement.FunctionManagement.Registered.examples.game': 'Game',
  'menu.GameManagement.FunctionManagement.Registered.examples.inventory': 'Inventory',
  'menu.GameManagement.FunctionManagement.Registered.examples.player': 'Player',
  'menu.GameManagement.FunctionManagement.Registered.examples.alertmanager': 'Alert Manager',
  'menu.GameManagement.FunctionManagement.Registered.examples.grafana': 'Grafana',
  'menu.GameManagement.FunctionManagement.Registered.examples.http': 'HTTP',
  'menu.GameManagement.FunctionManagement.Registered.examples.prom': 'Prometheus',
  'menu.GameManagement.FunctionManagement.FunctionInstances': 'Instance Management',
  'menu.GameManagement.FunctionManagement.FunctionAssignments': 'Permission Assignments',
  'menu.GameManagement.FunctionManagement.FunctionPacks': 'Pack Management',
  'menu.GameManagement.FunctionManagement.OpenAPISpec': 'OpenAPI Spec',
  'menu.GameManagement.GameFunctionsLegacy': 'Legacy Function Center',
  'menu.GameManagement.ComponentManagement': 'Component Management',
  'menu.GameManagement.CreateVirtualObject': 'Create Virtual Object',
  'menu.GameManagement.EditVirtualObject': 'Edit Virtual Object',
  'menu.GameManagement.GameAssignments': 'Function Assignments',
  'menu.GameManagement.GamePacks': 'Pack Management',

  // Operations management
  'menu.Operations': 'Operations',
  'menu.Operations.Approvals': 'Approval Management',
  'menu.Operations.Audit': 'Audit Logs',
  'menu.Operations.OperationLogs': 'Operation Logs',
  'menu.Operations.Registry': 'Service Registry',
  'menu.Operations.Servers': 'Servers',
  'menu.Operations.Configs': 'Configs',
  'menu.Operations.Platforms': 'Third-party Platforms',
  'menu.Operations.Storage': 'Object Storage',

  // Ops
  'menu.Ops': 'Operations',
  'menu.Ops.Services': 'Services',
  'menu.Ops.Health': 'Health',
  'menu.Ops.Nodes': 'Node Maintenance',
  'menu.Ops.Jobs': 'Jobs',
  'menu.Ops.Alerts': 'Alerts',
  'menu.Ops.RateLimits': 'Rate Limits',
  'menu.Ops.MQ': 'Message Queue',
  'menu.Ops.Certificates': 'Certificates',
  'menu.Ops.Notifications': 'Notifications',
  'menu.Ops.AnalyticsFilters': 'Sampling Control',
  'menu.Ops.Backups': 'Backups',
  'menu.Ops.Maintenance': 'Maintenance',

  // Analytics
  'menu.Analytics': 'Analytics',
  'menu.Analytics.Overview': 'Overview',
  'menu.Analytics.Retention': 'Retention',
  'menu.Analytics.Realtime': 'Realtime',
  'menu.Analytics.Behavior': 'Behavior',
  'menu.Analytics.Payments': 'Payments',
  'menu.Analytics.Levels': 'Levels',
  'menu.Analytics.Attribution': 'Attribution',
  'menu.Analytics.Segments': 'Segments',

  // Function Management - New independent pages
  'menu.Functions': 'Function Management',
  'menu.Functions.FunctionList': 'Function List',
  'menu.Functions.FunctionDetail': 'Function Detail',

  // Legacy menu items (for backward compatibility)
  'menu.GameAssignments': 'Assignments',
  'menu.GM': 'Game Management',
  'menu.GM.Approvals': 'Approvals',
  'menu.GM.Functions': 'Functions',
  'menu.GM.Games': 'Games',
  'menu.GM.Assignments': 'Assignments',
  'menu.GM.Audit': 'Audit',
  'menu.GM.Packs': 'Packs',
  'menu.GM.Registry': 'Registry',
  'menu.GameMgmt': 'Game Management',
  'menu.GameMgmt.Games Meta': 'Games Meta',
  'menu.GameMgmt.Entities': 'Entity Management',
  // Backward-compat for older route keys that may still reference GM.Games Meta
  'menu.GM.Games Meta': 'Games Meta',
  'menu.Permissions': 'Permission Management',
  'menu.Permissions.Roles': 'Role Management',
  'menu.Permissions.Users': 'User Management',
  'menu.Permissions.Config': 'Permission Config',
  'menu.welcome': 'Welcome',
  'menu.login': 'Login',
  // Function example translations
  'menu.GameManagement.FunctionManagement.Registered.examples': 'Examples',
  'menu.GameManagement.FunctionManagement.Registered.examples.examples': 'Examples',
  'menu.GameManagement.FunctionManagement.Registered.examples.examples.analytics.player_retention': 'Player Retention',
  'menu.GameManagement.FunctionManagement.Registered.examples.examples.chat.send_message': 'Send Message',
  'menu.GameManagement.FunctionManagement.Registered.examples.examples.config.set': 'Set Config',
  'menu.GameManagement.FunctionManagement.Registered.examples.examples.game.trigger_event': 'Trigger Event',
  'menu.GameManagement.FunctionManagement.Registered.examples.examples.inventory.get': 'Get Item',
  'menu.GameManagement.FunctionManagement.Registered.examples.examples.player.ban_batch': 'Batch Ban',
  'menu.GameManagement.FunctionManagement.Registered.examples.examples.player.create': 'Create Player',
  'menu.GameManagement.FunctionManagement.Registered.examples.examples.player.delete': 'Delete Player',
  'menu.GameManagement.FunctionManagement.Registered.examples.examples.player.get': 'Get Player',
  'menu.GameManagement.FunctionManagement.Registered.examples.examples.player.get_stats_legacy': 'Get Player Stats (Legacy)',
  'menu.GameManagement.FunctionManagement.Registered.examples.examples.player.list': 'List Players',
  'menu.GameManagement.FunctionManagement.Registered.examples.examples.player.update_profile': 'Update Profile',
  'menu.GameManagement.FunctionManagement.Registered.examples.examples.player.upload_avatar': 'Upload Avatar',
  'menu.GameManagement.FunctionManagement.Registered.packs': 'Function Packs',
  'menu.GameManagement.FunctionManagement.Registered.packs.packs.alertmanager.list_alerts': 'AlertManager List Alerts',
  'menu.GameManagement.FunctionManagement.Registered.packs.packs.grafana.search_dashboards': 'Grafana Search Dashboards',
  'menu.GameManagement.FunctionManagement.Registered.packs.packs.http.generic_invoke': 'HTTP Generic Invoke',
  'menu.GameManagement.FunctionManagement.Registered.packs.packs.player.ban': 'Player Ban',
  'menu.GameManagement.FunctionManagement.Registered.packs.packs.prom.query': 'Prometheus Query',
  'menu.GameManagement.FunctionManagement.Registered.packs.packs.prom.query_range': 'Prometheus Query Range',
};
