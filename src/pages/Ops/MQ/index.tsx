      ...
      >
        <Descriptions bordered column={1} size="small">
          <Descriptions.Item label="类型">
            {typ ? (
              <Tag color={typ === 'redis' ? 'green' : typ === 'kafka' ? 'blue' : 'default'}>
                {typ}
              </Tag>
            ) : (
              '-'
            ...
            <>
              ...
              {info?.lengths && (
                <Descriptions.Item label="长度">
                  {Object.entries(info.lengths).map(([k, v]: any) => (
                    <span key={k}>
                      <Tag>{k}</Tag> {String(v)}&nbsp;&nbsp;
                    </span>
                  ))}
                  ...
                  <div>
                    {info.groups.map((g: any, idx: number) => (
                      <div key={idx} style={{ marginBottom: 6 }}>
                        <Tag color="blue">{g.stream}</Tag> 组：<Tag>{g.name}</Tag> 消费者：
                        {g.consumers} 待处理：{g.pending} Lag：{g.lag ?? '-'}
                      </div>
                    ))}
                  </div>
                </Descriptions.Item>
