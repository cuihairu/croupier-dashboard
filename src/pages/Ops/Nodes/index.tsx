...
export default function OpsNodesPage() {
  ...
  const data = useMemo(() => {
    return (rows || []).filter((a) => {
      ...
      if (q) {
        const s = `${a.agent_id} ${a.ip || ''} ${a.rpc_addr || ''} ${a.type || ''} ${
          a.version || ''
        }`.toLowerCase();
  ...
  const cols: ColumnsType<RegistryAgent> = [
    ...
    { title: 'Functions', dataIndex: 'functions', width: 100 },
    {
      ...
      dataIndex: 'healthy',
      width: 90,
      render: (v) => (v ? <Tag color="green">healthy</Tag> : <Tag>expired</Tag>),
    },
    ...
    { title: 'RPC Addr', dataIndex: 'rpc_addr', width: 220, ellipsis: true },
    {
      ...
      render: (_: any, r) => (
        <Space>
          <Button size="small" onClick={() => drain(r.agent_id)}>
            下线
          </Button>
          <Button size="small" onClick={() => undrain(r.agent_id)}>
            取消下线
          </Button>
          <Button size="small" onClick={() => restart(r.agent_id)}>
            重启
          </Button>
        </Space>
      ),
  ...
  const games = Array.from(new Set(rows.map((r) => r.game_id).filter(Boolean))).map((v) => ({
    label: v,
    value: v,
  }));
