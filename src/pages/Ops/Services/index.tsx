...
export default function OpsServicesPage() {
  ...
  const data = useMemo(() => {
    return (rows || []).filter((r) => {
      ...
      if (filter.q) {
        ...
        const s = `${r.id} ${r.name} ${r.address || ''} ${r.type || ''} ${r.version || ''} ${
          r.region || ''
        } ${r.zone || ''}`.toLowerCase();
  ...
  const columns: ColumnsType<OpsService> = [
    ...
    { title: 'Functions', dataIndex: 'functionsCount', width: 80, render: (v) => v || 0 },
    {
      title: 'Processes',
      width: 90,
      render: (_: any, r) => r?.metadata?.processesCount ?? r?.metadata?.processes?.length ?? 0,
    },
    {
      title: 'Last Seen',
      dataIndex: 'lastSeen',
      ...
      >
        <Table<OpsService>
          ...
          onRow={(rec) => ({
            onClick: () => {
              setDetail(rec);
              setProcesses((rec?.metadata?.processes || []) as any);
            },
          })}
            ...
            <Card title="基础信息" size="small">
              <Descriptions column={2} size="small">
                ...
                <Descriptions.Item label="Functions">
                  {detail.functionsCount || 0}
                </Descriptions.Item>
                <Descriptions.Item label="最后活跃">
                  {detail.lastSeen ? new Date(detail.lastSeen).toLocaleString() : '-'}
                </Descriptions.Item>
              </Descriptions>
            </Card>
                ...
                <Table<AgentProcess>
                  ...
                  columns={[
                    ...
                    },
                    {
                      ...
                      dataIndex: 'functions',
                      width: 90,
                      render: (v: any, r: any) => v ?? (r?.function_ids?.length || 0),
                    },
                    {
                      title: 'last_seen',
                      dataIndex: 'last_seen_unix',
                      width: 150,
                  ...
                  expandable={{
                    ...
                    rowExpandable: (r) =>
                      Array.isArray(r?.function_ids) && r.function_ids.length > 0,
                  }}
                />
              </Card>
            )}
              ...
              {detail.labels ? (
                <Space wrap>
                  {Object.entries(detail.labels).map(([key, value], index) => {
                    ...
                    return (
                      <Tag key={key} color={colors[index % colors.length]}>
                        {key}: {value}
                      </Tag>
                    );
                  })}
