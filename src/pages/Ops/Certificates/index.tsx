...
export default function OpsCertificatesPage() {
  ...
  const columns: ColumnsType<Certificate> = [
    ...
    { title: '有效期至', dataIndex: 'valid_to', width: 160, render: (v) => fmt(v) },
    {
      ...
      dataIndex: 'days_left',
      width: 100,
      render: (_: any, r) => daysTag(r.days_left, r.status),
    },
    {
      title: '状态',
      dataIndex: 'status',
      ...
      render: (_: any, r) => (
        <Space>
          ...
          <Tooltip title={r.error_msg || ''}>
            <span>{r.error_msg ? <Tag color="red">错误</Tag> : null}</span>
          </Tooltip>
        </Space>
      ),
    },
...
const AddDomainModal: React.FC<{
  open: boolean;
  onClose: () => void;
  onOk: (v: { domain: string; port?: number; alert_days?: number }) => void;
}> = ({ open, onClose, onOk }) => {
  const [form] = Form.useForm();
  useEffect(() => {
    if (open) form.setFieldsValue({ port: 443, alert_days: 30 });
  }, [open]);
  return (
    <Modal
      open={open}
