...
export default function OpsMaintenancePage() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const r = await fetchOpsMaintenance();
      setRows(r?.windows || []);
    } finally {
      setLoading(false);
    }
  ...
  const saveAll = async (ws: Window[]) => {
    await saveOpsMaintenance({ windows: ws });
    message.success('已保存');
    load();
  };
    ...
    <div style={{ padding: 24 }}>
                ...
                <Space>
                  <Button
                    ...
                    onClick={() => {
                      form.setFieldsValue({
                        ...
                        range:
                          r.start && r.end
                            ? [new Date(r.start) as any, new Date(r.end) as any]
                            : undefined,
                      });
                      setOpen(true);
                    }}
                  ...
                  <Button
                    ...
                    onClick={() =>
                      Modal.confirm({
                        title: '删除公告',
                        onOk: () => saveAll(rows.filter((x) => x.id !== r.id)),
                      })
                    }
                  >
                    删除
      ...
      >
        <Form form={form} layout="vertical">
          ...
          <Form.Item label="Game" name="game_id">
            <Input />
          </Form.Item>
          ...
          <Form.Item label="拦截写操作" name="block_writes" valuePropName="checked">
            <Switch />
          </Form.Item>
        </Form>
      </Modal>
