  ...
  return (
    <PageContainer>
        ...
        extra={
          <Space>
            ...
            <Input
              ...
              style={{ width: 160 }}
            />
            {cfg?.grafana_explore_url && (
              <Button onClick={() => window.open(cfg.grafana_explore_url, '_blank')}>
                打开 Grafana
              </Button>
            )}
            {cfg?.alertmanager_url && (
              <Button onClick={() => window.open(cfg.alertmanager_url + '/#/alerts', '_blank')}>
                打开 AM
              </Button>
            )}
            <Button
      ...
      >
        <Table
          rowKey={(r) =>
            `${r.service || ''}|${r.instance || ''}|${r.summary || ''}|${r.starts_at || ''}`
          }
          loading={loading}
          dataSource={data}
          columns={columns}
      ...
      >
        <Table
          ...
          columns={[
            ...
            {
              title: '时间',
              render: (_: any, r: any) => `${r.starts_at || ''} -> ${r.ends_at || ''}`,
            },
      ...
      <Drawer title="告警详情" width={720} open={!!detail} onClose={() => setDetail(null)}>
        {detail && (
          <Space direction="vertical" style={{ width: '100%' }}>
            ...
            <div>
              <b>开始时间:</b> {detail.starts_at || '-'} <b>时长:</b> {detail.duration || '-'}
            </div>
            <div>
              <b>状态:</b>{' '}
            ...
            <Space>
              ...
              {((detail.annotations || {}) as any).runbook_url && (
                <Button
                  onClick={() =>
                    window.open(String((detail.annotations as any).runbook_url), '_blank')
                  }
                >
                  打开 Runbook
                </Button>
