...
export default function OpsJobsPage() {
  ...
  const columns: ColumnsType<OpsJob> = [
    ...
    { title: '操作者', dataIndex: 'actor', width: 140 },
    {
      ...
      key: 'ge',
      width: 160,
      render: (_: any, r: any) => `${r.game_id || ''}/${r.env || ''}`,
    },
    {
      title: '耗时',
      ...
      >
        {detail && (
          <Space direction="vertical" style={{ width: '100%' }}>
            ...
            <div>
              <b>操作者:</b> {detail.actor || '-'}
            </div>
            <div>
              <b>游戏/环境:</b> {(detail.game_id || '') + '/' + (detail.env || '')}
            </div>
            <div>
              <b>服务地址:</b> {detail.rpc_addr || '-'}
            </div>
            <div>
              <b>Trace:</b> {detail.trace_id || '-'}
            </div>
            <div>
              <b>开始时间:</b>{' '}
            ...
            <div>
              <b>结束时间:</b> {detail.ended_at ? new Date(detail.ended_at).toLocaleString() : '-'}
            </div>
            <div>
              <b>耗时:</b>{' '}
