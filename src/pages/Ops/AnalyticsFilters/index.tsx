...
export default function AnalyticsFiltersPage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const arr = (r?.games || []).map((g: any) => ({
          id: String(g.id),
          name: g.display_name || g.name || String(g.id),
        }));
        setGames(arr);
      } catch {}
    })();
  ...
  const load = async () => {
    ...
    try {
      ...
      if (r?.sample_global != null) setSampleGlobal(Number(r.sample_global));
      else setSampleGlobal(100);
    } catch {
      message.error('加载失败');
    } finally {
      setLoading(false);
