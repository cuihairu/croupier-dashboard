...
export default function OpsHealthPage() {
  ...
  const save = async () => {
    try {
      const rows = checks.slice();
      const v = await form.validateFields();
      const idx = rows.findIndex((x) => x.id === v.id);
      if (idx >= 0) rows[idx] = v;
      else rows.push(v);
      await saveOpsHealthChecks(rows);
      message.success('已保存');
      setOpen(false);
      load();
    } catch {}
  ...
  const remove = async (id: string) => {
    try {
      const rows = checks.filter((x) => x.id !== id);
      await saveOpsHealthChecks(rows);
      message.success('已保存');
      load();
    } catch (e: any) {
    ...
    <PageContainer>
          ...
          columns={[
            ...
            { title: '区域', dataIndex: 'region' },
            {
              ...
              render: (_: any, r: any) => {
                ...
                return (
                  <Space>
                    <Tag color={st.ok ? 'green' : 'red'}>{st.ok ? 'OK' : 'FAIL'}</Tag>
                    <span>{st.latency_ms}ms</span>
                    {st.error ? <span style={{ color: '#999' }}>{st.error}</span> : null}
                  </Space>
                );
              },
            },
            {
              ...
              render: (_: any, r: any) => (
                <Space>
                  ...
                  <Button size="small" onClick={() => runNow(r.id)}>
                    重测
                  </Button>
                  <Button
                    size="small"
                    danger
                    onClick={() => Modal.confirm({ title: '删除检查', onOk: () => remove(r.id) })}
                  >
                    删除
                  </Button>
                </Space>
      ...
      >
        <Form form={form} layout="vertical" initialValues={{ interval_sec: 60, timeout_ms: 1000 }}>
          ...
          <Form.Item label="目标" name="target" rules={[{ required: true }]}>
            <Input placeholder="URL 或 host:port 或 redis://" />
          </Form.Item>
          <Form.Item label="期望" name="expect">
            <Input placeholder="可选：例如 http 期望状态码 200" />
          </Form.Item>
          <Form.Item label="频率(秒)" name="interval_sec">
