...
- [x] 补齐前端埋点：页面打开（工作台列表/控制台列表/控制台详情/编辑器）与运行时渲染失败埋点（TASK-039 完成）
- [x] 新增 `docs/workspace-regression-checklist.md`，覆盖角色/页面/发布链路回归项（TASK-031 完成）
- [x] 新增 `workspace` 统一错误解析模块并接入关键页面错误提示（Workspaces/WorkspaceEditor/Console/Renderer）（TASK-007 前端子项完成）
- [x] 后端 `workspace` 接口统一错误响应结构为 `code/message/request_id`，并补齐保存接口基础字段校验（TASK-007 后端子项部分完成）
- [x] 后端新增 `GET /workspaces/:objectKey/versions` 与 `POST /workspaces/:objectKey/rollback`，并接入版本快照持久化（save/publish/unpublish）（TASK-021/TASK-022 后端接口完成）
- [x] 后端 `workspace` 接口接入权限校验（read/edit/publish/rollback/delete），拒绝时统一返回 `forbidden`（TASK-025 workspace 范围完成）
- [x] 后端 `workspace` 关键动作接入审计日志（save/publish/unpublish/rollback/delete），落地操作者/对象/动作/时间/摘要及 `request_id`（TASK-027 workspace 范围完成）
- [x] 配置层补齐 `workspace:*` 权限定义与角色映射（super_admin/admin/tech_lead/backend_lead/developer/frontend_developer/tester/viewer）（TASK-024 后端配置完成）
- [x] 新增 `docs/workspace-error-contract.md` 与 `croupier/docs/api/workspace.md`，固化错误结构、错误码与前后端映射约定（TASK-007 规范文档完成）
- [x] 新增 `tests/workspace/service-api.test.ts` 与 `tests/workspace/errors.test.ts`，补齐 Workspace service 成功/失败分支与错误解析测试（TASK-028 完成）
- [x] 新增 `tests/workspace/tabContentRenderer.test.tsx`，覆盖 tabs/list/form/detail/form-detail 渲染分发测试（TASK-029 完成）
- [x] 修复 `jest` 运行环境（移除 `@umijs/max/test` 强依赖、补齐 setup 配置），已跑通 Workspace 相关 13 条测试（TASK-028/TASK-029 验证完成）
- [x] 新增 `tests/workspace/flow.test.ts`，覆盖最小闭环链路（保存草稿 → 发布 → 控制台读取 → 回滚）与关键失败链路（发布失败后草稿仍可读）（TASK-030 完成）
- [x] 后端字段契约对齐完成：`WorkspaceConfig`/保存请求补齐 `description/status/version`，列表/详情/已发布查询返回版本号，状态字段统一归一（TASK-001 后端完成）
- [x] 输出并固化 Workspace 状态机（`draft/published/archived`）及动作转换规则到后端文档；后端状态解析按发布标记优先归一（TASK-002 完成）
- [x] 完成 `clone/import/export` 能力核对：后端无独立接口，前端主流程不暴露对应入口，版本/回滚已对齐正式 API（TASK-006 完成）
- [x] `TemplateManager` 收敛到 V1 模板能力（仅 `tabs + form-detail/list/form/detail`），导入模板增加 V1 校验，编辑器标题去“编排”语义（TASK-003 完成）
- [x] `Console/Detail` 增加稳定异常分支：非法 `objectKey`、`workspace_not_found`、`forbidden`、通用加载失败重试；并继续复用 `WorkspaceRenderer + useWorkspaceConfig` 运行模型（TASK-010 完成）
...
- [x] 梳理前端现有 `WorkspaceConfig` 字段
- [x] 梳理后端现有 `WorkspaceConfig` 字段
- [x] 列出差异字段清单
- [x] 确认 `description` 是否纳入后端正式字段
- [x] 确认 `version` 和 `status` 的正式定义
- [x] 输出统一字段表
- [x] 修改前端类型定义
- [x] 修改后端 API 定义
- [x] 确认返回示例 JSON
...
- [x] 定义 `draft / published / archived` 状态
- [x] 定义“保存草稿”状态转换
- [x] 定义“发布”状态转换
- [x] 定义“取消发布”状态转换
- [x] 定义“回滚”状态转换
...
- [x] 核对 `import` 是否后端支持
- [x] 核对 `export` 是否后端支持
- [x] 核对 `version list` 是否后端支持
- [x] 核对 `rollback` 是否后端支持
- [x] 未实现接口先移除前端入口
- [x] 已实现接口统一到正式 service
...
- [x] 在编辑器中只保留 `tabs`
- [x] 在 tab 中只保留 `list/form/detail/form-detail`
- [x] 隐藏 `sections`
- [x] 隐藏 `wizard`
- [x] 隐藏 `dashboard`
- [x] 隐藏 `custom`
- [x] 隐藏 `chart`
- [x] 隐藏 `VisualNodeEditor` 入口或明确标记规划中
...
- [x] 明确 V1 不交付节点执行编排
- [x] 隐藏入口或增加明显提示
- [x] 避免用户误以为 `condition` / `transform` 会真正生效
- [x] 如果保留代码，标记为内部实验能力，不对业务开放
