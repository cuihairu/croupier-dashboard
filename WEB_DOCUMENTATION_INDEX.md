...
- `docs/PAGE_SCHEMA_MIGRATION.md`
  - 后台配置 UI 的 schema 化重构进度（Assignments / Packs / Functions Directory / Operations Configs）
- `docs/UI_REFACTOR_ACCEPTANCE_CHECKLIST.md`
  - 最终验收清单（系统配置 / 函数管理 / 函数详情 + 回归项）
- `docs/UI_REFACTOR_FINAL_REPORT.md`
  - 重构最终报告（目标、完成项、统一约定、快速验收顺序）
- `src/pages/Assignments/README.md`
  - 分配页面快速导读（index/hook/schema/renderer/columns）
- `src/pages/Packs/README.md`
  - 包管理页面快速导读（薄入口 + hook + 组件拆分）
- `src/pages/Functions/Directory/README.md`
  - 函数管理页快速导读（列表动作 + 详情抽屉动作）
- `src/pages/Functions/README.md`
  - 函数模块总览（函数管理 + 函数详情）
- `src/pages/Operations/Configs/README.md`
  - 系统配置页快速导读（查询/编辑/版本/Diff/回滚）
...
A: 在浏览器开发者工具 Network 标签中查看请求和响应, 确保后端服务在 http://localhost:18780
